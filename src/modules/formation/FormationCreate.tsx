...
const ANIMATEURS = ['Mon premier', 'Pérames', 'Coordinateur', 'Chefs de troupe', 'AGA', 'ASA', 'ARO', 'AJO', 'Autres'];
...
// Composant éditeur riche réutilisable
const RichEditor = ({
    content,
    onChange,
    placeholder,
...
export const FormationCreate = () => {
    ...
    const [troupeSections, setTroupeSections] = useState<TroupeSection[]>([
        { id: Math.random().toString(36).substring(2, 9), nom_troupe: 'SAGA', sous_theme: '', contenu: '' },
    ]);
    ...
    const createMutation = useMutation({
        mutationFn: async () => {
            ...
            if (form.type_lecon === 'troupe' && troupeSections.length > 0) {
        ...
        onSuccess: (lecon) => {
            ...
            showNotification('Leçon créée avec succès !', 'success');
            navigate(`/formation/${lecon.id}`);
        },
        onError: (err) => {
            console.error(err);
            showNotification("Erreur lors de la création. Vérifiez la console.", 'error');
        },
    });
    ...
    const handleSubmit = () => {
        ...
        if (form.type_lecon === 'unique' && !form.contenu_unique.replace(/<[^>]*>/g, '').trim()) {
            showNotification('Veuillez saisir le contenu de la leçon', 'error');
            return;
        }
        ...
        <div className="min-h-screen bg-theme-bg text-theme-text-primary flex flex-col font-sans">
            ...
            <header className="bg-theme-bg/90 backdrop-blur-xl px-4 py-3 flex items-center justify-between border-b border-theme-border sticky top-0 z-50">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <span className="block font-black text-theme-text-primary text-base leading-none uppercase tracking-tighter">Nouvelle leçon</span>
                        <span className="text-[8px] font-bold text-theme-accent-end uppercase tracking-[0.25em] opacity-80">Création</span>
                    </div>
                </div>
                <button
            ...
            <main className="max-w-2xl mx-auto w-full p-4 pb-28 space-y-5">
                ...
                <section className="bg-theme-surface rounded-2xl border border-theme-border p-4 space-y-4">
                    <h2 className="font-black text-sm text-theme-text-primary uppercase tracking-wide">Informations générales</h2>
                    ...
                    <div>
                        <label className={labelClass}>Thème général *</label>
                        <input type="text" placeholder="Ex : La prière intercessive" value={form.theme_general} onChange={e => setForm(p => ({ ...p, theme_general: e.target.value }))} className={inputClass} />
                    </div>

                    <div>
                        <label className={labelClass}>Objectif de la leçon *</label>
                ...
                {/* Type de leçon */}
                <section className="bg-theme-surface rounded-2xl border border-theme-border p-4 space-y-4">
                    <h2 className="font-black text-sm text-theme-text-primary uppercase tracking-wide">Type de leçon</h2>
                    <div className="flex bg-theme-bg p-1.5 rounded-xl border border-theme-border gap-1">
                        {(['unique', 'troupe'] as const).map(type => (
                            ...
                            >
                                ...
                                {type === 'unique' ? 'Leçon Unique' : 'Par Troupe'}
                            </button>
                        ))}
                    </div>
                </section>
                ...
                {form.type_lecon === 'unique' ? (
                    <section className="bg-theme-surface rounded-2xl border border-theme-border p-4 space-y-3">
                        <h2 className="font-black text-sm text-theme-text-primary uppercase tracking-wide">Contenu de la leçon</h2>
                        <RichEditor
                            content={form.contenu_unique}
                            onChange={html => setForm(p => ({ ...p, contenu_unique: html }))}
                            placeholder="Commencez à rédiger le contenu de la leçon..."
                        />
                    </section>
                ) : (
                        ...
                        {troupeSections.map((section, idx) => (
                            <div key={section.id} className="bg-theme-surface rounded-2xl border border-theme-border p-4 space-y-3">
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    ...
                                    <div>
                                        <label className={labelClass}>Sous-thème *</label>
