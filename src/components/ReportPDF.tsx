        ...
        <div className="relative z-10 flex flex-col w-full" style={{ boxSizing: 'border-box' }}>
          ...
          <div className="flex justify-between items-start mb-8 w-full" style={{ fontSize: '11.5pt', color: '#000000', boxSizing: 'border-box' }}>
            <div className="text-left font-bold uppercase pt-1" style={{ maxWidth: '400px' }}>
              OBJET : RAPPORT {data.reportType.toUpperCase()} DES ACTIVITÉS
            </div>
          ...
          <div className="space-y-4 w-full" style={{ boxSizing: 'border-box', fontSize: '11.5pt', color: '#000000' }}>
            ...
            <p style={{ textAlign: "justify", textJustify: "inter-word", lineHeight: "1.8", textIndent: "35px", margin: 0 }}>
              Nous venons par la présente vous soumettre le rapport <strong>{data.reportType}</strong> des activités de la structure <strong>{data.structureName}</strong>, concernant l'année initiatique <strong>{data.anneeInitiatique}</strong>.
              Ce document présente l'état de nos effectifs ainsi qu'une synthèse des activités réalisées et de celles à venir.
            </p>
            ...
            <p style={{ textAlign: "justify", textJustify: "inter-word", lineHeight: "1.8", textIndent: "35px", marginBottom: "12px" }}>
              ...
              Ces membres sont répartis au sein de nos troupes avec {data.effectifs?.anges?.saga || 0} membres au SAGA, {data.effectifs?.anges?.sao || 0} membres au SAO et {data.effectifs?.anges?.sara || 0} membres au SARA.
              ...
              {data.effectifs?.anges?.systeme_fonctions?.length > 0 ? `, incluant les rôles de responsabilité suivants : ${data.effectifs.anges.systeme_fonctions.map((f: any) => `${f.nom} (${f.count})`).join(', ')}` : ''}.
            </p>
            ...
            <p style={{ textAlign: "justify", textJustify: "inter-word", lineHeight: "1.8", textIndent: "35px", marginBottom: "12px" }}>
              ...
              {data.effectifs?.archanges?.systeme_fonctions?.length > 0 ? `, avec les rôles suivants : ${data.effectifs.archanges.systeme_fonctions.map((f: any) => `${f.nom} (${f.count})`).join(', ')}` : ''}.
            </p>
